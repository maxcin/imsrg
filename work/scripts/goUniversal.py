...
path_to_ME = path.expanduser("~/IMSRG/ME/")
 ...
 for reference in ['%s%d'%(ELEM[Z],A)]:
  ...
  for e in [6]:
   for hw in [16]:
     ...
     #ARGS['3bme'] = path_to_ME+f'NO2B_ThBME_EM1.8_2.0_3NFJmax15_JJmax15_IS_hw16_ms18_36_24.stream.bin '
     ARGS['2bme'] = path_to_ME+f"TwBME-HO_NN-only_N3LO_EM500_srg1.8_hw16_emax14_e2max28.me2j.gz"
     #ARGS['3bme'] = path_to_ME+f'NO2B_ThBME_EM1.8_2.0_3NFJmax15_JJmax15_IS_hw16_ms18_36_24.stream.bin '
     #ARGS['3bme_c3'] = path_to_ME+f'NO2B_ThBME_EM1.8_2.0_3NFJmax15_JJmax15_IS_hw16_ms18_36_24.stream.bin '

     ARGS['LECs'] = 'EM1.8_2.0'
     ARGS['file2e1max'] = '14 file2e2max=28'
...
#     ARGS['method'] = method
     ...
     #ARGS['perturbative_triples'] = "true"
...
#     ARGS['Operators'] = 'GamowTeller'
    ...
    ### Make an estimate of how much time to request. Only used for slurm at the moment.
     ...
     if 'approx_3f2' in ARGS:
          if ARGS['approx_3f2'] == "true":
              jobname  = '%s_%s_%s_%s_%s_e%s_E%s_s%s_hw%s_A%s' %(ARGS['valence_space'], ARGS['LECs'],ARGS['method'],"3f2",ARGS['reference'],ARGS['emax'],ARGS['e3max'],ARGS['smax'],ARGS['hw'],ARGS['A'])
          else:
     ...
     else: jobname  = '%s_%s_%s_%s_e%s_E%s_s%s_hw%s_A%s' %(ARGS['valence_space'], ARGS['LECs'],ARGS['method'],ARGS['reference'],ARGS['emax'],ARGS['e3max'],ARGS['smax'],ARGS['hw'],ARGS['A'])

     jobname  = '%s_%s_%s_%s_e%s_E%s_s%s_hw%s_A%s' %(ARGS['valence_space'], ARGS['LECs'],ARGS['method'],ARGS['reference'],ARGS['emax'],ARGS['e3max'],ARGS['smax'],ARGS['hw'],ARGS['A'])
     logname = jobname + datetime.fromtimestamp(time()).strftime('_%y%m%d%H%M.log')
    #  jobname = "printmat_"+jobname
  ### Some optional parameters that we probably want in the output name if we're using them
    ...
    #  if 'eta_criterion' in ARGS: jobname += '_eta%s'%(ARGS['eta_criterion'])
     ...
     if 'basis' in ARGS :
          if ARGS['basis'] == "NAT" and 'NAT_type' in ARGS:
              jobname += "_" +ARGS['NAT_type']+ARGS['basis']
          else: jobname += "_" + ARGS['basis']
