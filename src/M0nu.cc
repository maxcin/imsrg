...
namespace M0nu
{
  ...
  { 
    ...
    for (int i = 0 ; i<momentum_mesh_size; i++)
    {
      ...
      for (int j = 0; j < momentum_mesh_size; j++)
      {
        ...
        overlap = xi*xi*xj*xj*wf*wfp;
        double w1 = pwd.getW(xi, xj, i, j, S, l, lp, J) ;
        I += wi * wj * overlap * w1;
      }
    }
    return I;
  ...
  std::unordered_map<uint64_t, double> PreCalculateM0nuIntegrals(int e2max, int Smin, int Smax, double hw, int l_diff,  PWD& pwd)
  {
        ...
        for (int l=0; l<=maxl-2*n; l++)
        {
          ...
          for (int np=tempminnp; np<=maxnp; np++)
          {
            ...
            else
            {
              ...
              int maxlp = std::min(l + l_diff, maxl);
  ...
  double GetM0nuIntegral(int e2max, int n, int l, int np, int lp, int J, int S, std::unordered_map<uint64_t, double> &IntList)
  {
    ...
    {
      std::cout<<"Integral value not precalculated..."<<std::endl;
  ...
  Operator GamowTeller(ModelSpace &modelspace, double Eclosure, std::string src)
  {
    ...
    pwd.setPotential([Eclosure](double p, double pp,  double z){return potential_closure(p,pp,z,Eclosure,GTFormFactor);}, "spin-spin");
    pwd.calcA(e2max,0);
    pwd.freeAngularMesh();
    std::cout << "Done precomputing A's." << std::endl;
    std::unordered_map<uint64_t, double> IntList = PreCalculateM0nuIntegrals(e2max, 0, 1, hw, 0, pwd); // pre-calculate the needed integrals over dp and dpp
                  ...
                  for (int Ncom = 0; Ncom <= tempmaxNcom; Ncom++)
                  {
                    ...
                    for (int lr = tempminlr; lr <= tempmaxlr; lr++)
                    {
                      ...
                      for (int Jrel = minJrel; Jrel <= maxJrel; Jrel++)
                      {
                        ...
                        integral += normJrel * normJrel * GetM0nuIntegral(e2max, nr, lr, npr, lr, Jrel,S, IntList);                              // grab the pre-calculated integral wrt dq and dr from the IntList of the modelspace class
  ...
  Operator Fermi(ModelSpace& modelspace, double Eclosure, std::string src)
  {
    ...
    pwd.initializeMomentumMesh(500);
    pwd.setMaxMomentum(25);
    pwd.setPotential([Eclosure](double p, double pp,  double z){return potential_closure(p,pp,z,Eclosure,FermiFormFactor);}, "central");
    pwd.calcA(e2max,0);
    pwd.freeAngularMesh();
    std::cout<<"Done precomputing A's."<<std::endl;
    std::unordered_map<uint64_t, double> IntList = PreCalculateM0nuIntegrals(e2max, 0, 1, hw, 0, pwd); // pre-calculate the needed integrals over dq and dr, for efficiency
    std::cout<<"Done precomputing integrals."<<std::endl;
    pwd.clearA();
    pwd.freeMomentumMesh();
    ...
    Operator Tensor(ModelSpace& modelspace, double Eclosure, std::string src)
    {
      ...
      pwd.initializeMomentumMesh(500);
      pwd.setMaxMomentum(25);
      pwd.setPotential([Eclosure](double p, double pp, double z)
                       {
                         double qsq = p*p+pp*pp-2*p*pp*z;
                         return (3/qsq)*potential_closure(p, pp, z, Eclosure, TensorFormFactor); 
                        },
                        "tensor");
    pwd.setPotential([Eclosure](double p, double pp, double z)
                       {
                         return -1*potential_closure(p, pp, z, Eclosure, TensorFormFactor); 
                        },
                       "spin-spin");
      pwd.calcA(e2max, 0);
      std::cout << "Done precomputing A's." << std::endl;
      pwd.freeAngularMesh();
      std::unordered_map<uint64_t, double> IntList = PreCalculateM0nuIntegrals(e2max, 1, 1, hw, 2, pwd); // pre-calculate the needed integrals over dq and dr, for efficiency
      std::cout << "Done precomputing integrals." << std::endl;
      pwd.freeMomentumMesh();
      pwd.clearA();
                    ...
                    for (int Ncom=0; Ncom<=tempmaxNcom; Ncom++ )
                    {
                      ...
                      for (int npr=0; npr<=(eps_cd-2*Ncom-Lam)/2; npr++)
                      {
                          ...
                          for (int Jrel = minJrel; Jrel<=maxJrel; Jrel++)
                          {
                            ...
                            integral += normJrel * normJrelp * GetM0nuIntegral(e2max, nr, lr, npr, lpr, Jrel, S, IntList);
                          }
                          sumMT += Df*Di*integral; // perform the Moshinsky transformation
                          sumMTas += Df*asDi*integral; // (anti-symmetric part)
                ...
                for (int nr = 0; nr <= tempmaxnr; nr++)
                {
                  ...
                  if ((npr >= 0) and (npr == floor(npr)))
                  {
                    ...
                    for (int Ncom = 0; Ncom <= tempmaxNcom; Ncom++)
                    {
                      ...
                      double integral = normJrel * normJrel * GetM0nuIntegral(e2max, nr, lr, npr, lr, Jrel, S, IntList); // grab the pre-calculated integral wrt dq and dr from the IntList of the modelspace class
                      sumMT += Df*Di*integral; // perform the Moshinsky transformation
