  ...
  Operator AnapoleMoment(ModelSpace& modelspace)
  {
    ...
    for (int i=0; i<norbits; ++i)
    {
      ...
      for (int j: As.OneBodyChannels.at({oi.l, oi.j2, oi.tz2}))
      {
        ...
        As.OneBody(j, i) = - AngMom::phase((oi.j2 - oj.j2) / 2)* As.OneBody(i, j); // Operator is imagniary and therefore antisymmetric, reduced ME need a phase under hermitian conjugation
      }
    }
    return As;
  }
