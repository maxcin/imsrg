...
class access
  ...
  template<typename eT> coot_inline static const typename enable_if2<is_not_cx<eT>::value, const eT&>::result alt_conj(const eT& X) { return X;            }
  template<typename eT> coot_inline static const typename enable_if2<    is_cx<eT>::value, const eT >::result alt_conj(const eT& X) { return std::conj(X); }
  };
