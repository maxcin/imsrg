...
// This kernel performs the shifted prefix-sum on each individual workgroup.
// This is the same as just running a regular prefix-sum kernel, except that
// `out_mem[i]` will store the total sum of elements in workgroup `i`.
// After running this, to finish prefix-sum on the entire memory, offsets for
// each workgroup need to be added.
__kernel
void
COOT_FN(PREFIX,shifted_prefix_sum_subgroups)(__global eT1* mem,
  ...
  aux_mem[local_offset    ] = (mem_offset     < n_elem) ? mem[global_mem_offset + mem_offset    ] : (eT1) 0;
  aux_mem[local_offset + 1] = (mem_offset + 1 < n_elem) ? mem[global_mem_offset + mem_offset + 1] : (eT1) 0;
  ...
  if (mem_offset + 1 < n_elem)
    {
    mem[global_mem_offset + mem_offset    ] = aux_mem[local_offset    ];
    mem[global_mem_offset + mem_offset + 1] = aux_mem[local_offset + 1];
    }
