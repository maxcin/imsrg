...
class op_max
  ...
  template<typename eT2, typename T1>
  inline static void apply(Cube<eT2>& out, const OpCube<T1, op_max>& in);

  template<typename out_eT, typename in_eT>
  inline static void apply_noalias(Cube<out_eT>& out, const Cube<in_eT>& A, const uword dim, const bool post_conv_apply);

  template<typename T1> inline static uword compute_n_rows(const OpCube<T1, op_max>& op, const uword in_n_rows, const uword in_n_cols, const uword in_n_slices);
  template<typename T1> inline static uword compute_n_cols(const OpCube<T1, op_max>& op, const uword in_n_rows, const uword in_n_cols, const uword in_n_slices);
  template<typename T1> inline static uword compute_n_slices(const OpCube<T1, op_max>& op, const uword in_n_rows, const uword in_n_cols, const uword in_n_slices);
